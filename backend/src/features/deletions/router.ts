...
import { deletionJobs, notificationOutbox, users, userTombstones } from '../../db/schema.js'
...
function claims(payload: JWTPayload, audience: string) {
  ...
  if (
    payload.aud !== audience || payload['token_use'] !== 'deletion' ||
...
export function createDeletionsRouter(config: {
  jwksUrl: string
  issuer: string
  service: string
}) {
  ...
  const result = db.transaction((tx) => {
    ...
    const tombstone = tx.select().from(userTombstones)
      .where(eq(userTombstones.userId, principal.userId)).get()
    ...
    tx.insert(userTombstones).values({
      userId: principal.userId, deletionJobId: principal.jobId, deletedAt: completedAt,
    }).run()
