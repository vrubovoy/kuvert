...
function Root() {
  ...
  const { data: profile } = useUserProfile(!auth.loading && auth.user !== null)

  useEffect(() => {
    document.documentElement.dataset.weekStart = profile?.weekStart ?? 'monday'
    return () => { delete document.documentElement.dataset.weekStart }
  }, [profile?.weekStart])
